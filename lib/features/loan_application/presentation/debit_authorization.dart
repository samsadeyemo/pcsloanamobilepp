...
class _DebitAuthorizationScreen
  ...
  /// Inspects the profile payload's `bank_details` block. If the user
  /// already has both an account number AND a bank name saved, we
  /// consider the bank already linked/verified — no need to show the
  /// picker/verify/save UI again, just skip straight to the next screen.
  void _loadBankDetails(Map<String, dynamic>? data) {
    if (data == null) return;

    final bankDetails = data['bank_details'];

    final accountNumberValue =
        (bankDetails is Map ? bankDetails['account_number'] : null)
                ?.toString() ??
            '';
    final bankNameValue =
        (bankDetails is Map ? bankDetails['bank_name'] : null)?.toString() ??
            '';

    final hasLinkedBank =
        accountNumberValue.isNotEmpty && bankNameValue.isNotEmpty;

    setState(() {
      _bankAlreadyLinked = hasLinkedBank;
      _accountNumberController.text = accountNumberValue;

      if (hasLinkedBank) {
        _selectedBankName = bankNameValue;
        _verificationStatus = 'success';
        _verificationMessage = 'Account verified successfully';
        final firstName = data['first_name']?.toString() ?? '';
        final lastName = data['last_name']?.toString() ?? '';
        _accountName = '$firstName $lastName'.trim();
      }
    });

    print('Loaded bank_details: $bankDetails');
    print('Bank already linked: $hasLinkedBank');

    if (hasLinkedBank) {
      // Bank already on file — skip the whole flow and go straight through.
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) context.go('/loan-disbursed-screen');
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // If we've detected the bank is already linked, we're navigating away
    // via the post-frame callback in _loadBankDetails — show a spinner
    // instead of flashing the full form.
    if (_bankAlreadyLinked) {
      return Scaffold(
        backgroundColor: const Color(0xFFFFFFFF),
        appBar: CustomLoanAppBar(title: 'Bank Authorization'),
        body: const Center(
